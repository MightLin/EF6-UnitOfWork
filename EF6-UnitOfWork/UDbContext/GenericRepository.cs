...
namespace EF6_UnitOfWork.UDbContext
{
	public class GenericRepository<TEntity> : QueryRepository<TEntity>, IRepository<TEntity> where TEntity : class
	{
		...
		//		public int Update(Expression<Func<TEntity, bool>> predicate, Expression<Func<TEntity, TEntity>> updateFactory)
		//		{
		//			if (predicate == null)
		//				throw new ArgumentNullException(nameof(predicate));
		//			if (updateFactory == null)
		//				throw new ArgumentNullException(nameof(updateFactory));
		//			return Db.Set<TEntity>().Where(predicate).Update(updateFactory);
		//		}
